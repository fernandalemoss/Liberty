// produto-modal.js — popup de detalhes do produto
// Inclua no final do body de todas as páginas com cards
...
#modal-fechar {
  ...
  float: right; margin: 12px 12px -40px 0;
  background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1);
  color: rgba(237,224,200,0.5); width: 36px; height: 36px;
  display: flex; align-items: center; justify-content: center;
  cursor: pointer; transition: .2s; z-index: 10; border-radius: 2px;
  flex-shrink: 0;
}
...
#modal-nome {
  ...
  letter-spacing: 2px; line-height: 1.2; margin-bottom: 18px;
}
...
#modal-preco-original {
  font-size: 16px; color: rgba(237,224,200,0.35);
  text-decoration: line-through; font-family: 'Jost', sans-serif;
}
#modal-preco-final {
  font-size: 28px; color: #c9a84c; font-weight: 500;
  font-family: 'Jost', sans-serif; letter-spacing: 1px;
}
#modal-economia {
  font-size: 12px; background: rgba(80,180,100,0.12);
  border: 1px solid rgba(80,180,100,0.25);
  color: #70c880; padding: 3px 10px; letter-spacing: 1px;
}
...
#modal-estoque-txt {
  font-size: 12px; letter-spacing: 1px; color: rgba(237,224,200,0.5);
}

#modal-desc-curta {
  font-size: 14px; color: rgba(237,224,200,0.55);
  line-height: 1.7; letter-spacing: .5px; margin-bottom: 4px;
}
...
#modal-desc-longa {
  font-size: 14px; color: rgba(237,224,200,0.45);
...
#modal-relacionados-titulo {
  font-family: 'Cormorant Garamond', serif;
  font-size: 18px; font-weight: 300; color: rgba(237,224,200,0.5);
  letter-spacing: 3px; text-transform: uppercase;
  margin-bottom: 20px;
}

#modal-relacionados-grid {
  display: grid;
...
.rel-card img {
  ...
  object-fit: contain; background: #0a0a0a; padding: 12px; display: block;
}
...
.rel-card-desc { font-size: 11px; color: rgba(237,224,200,0.35); margin-top: 2px; letter-spacing:.5px;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
@media (max-width: 700px) {
  ...
  #modal-relacionados-grid { grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); }
}
`;
...
async function abrirModal(produtoId) {
  ...
  catEl.textContent    = catNome;
  subcatEl.textContent = subNome;
  catEl.style.display    = catNome ? 'inline' : 'none';
  subcatEl.style.display = subNome ? 'inline' : 'none';
  ...
  const elOriginal = document.getElementById('modal-preco-original');
  const elFinal    = document.getElementById('modal-preco-final');
  const elEcon     = document.getElementById('modal-economia');

  elFinal.textContent = fmtBRL(precoFinal);

  if (desconto > 0) {
    elOriginal.textContent = fmtBRL(precoOriginal);
    elOriginal.style.display = 'inline';
    elEcon.textContent = `Economia de ${fmtBRL(economia)}`;
    elEcon.style.display = 'inline';
  } else {
    elOriginal.style.display = 'none';
    elEcon.style.display     = 'none';
  }
  ...
  const descLongaWrap = document.getElementById('modal-desc-longa-wrap');
  if (p.descricao_longa) {
    document.getElementById('modal-desc-longa').textContent = p.descricao_longa;
    descLongaWrap.style.display = 'block';
  } else {
    descLongaWrap.style.display = 'none';
  }

  // ── BOTÃO WHATSAPP ──
  const waLink = `https://wa.me/5599999999999?text=${encodeURIComponent(`Olá! Tenho interesse no perfume ${p.nome}.`)}`;
  document.getElementById('modal-btn-wa').href = estoque === 0 ? '#' : waLink;
  document.getElementById('modal-btn-wa').style.opacity = estoque === 0 ? '0.4' : '1';
  document.getElementById('modal-btn-wa').style.pointerEvents = estoque === 0 ? 'none' : 'auto';
  if (estoque === 0) {
    document.getElementById('modal-btn-wa').textContent = 'Produto esgotado';
  }
...
async function carregarRelacionados(p) {
  if (!p.subcategorias) return;

  const { data: relacionados } = await db
    .from('produtos')
    .select('id, nome, descricao, preco, imagem, desconto')
    .eq('ativo', true)
    .eq('subcategoria_id', p.subcategoria_id)
    .neq('id', p.id)
    .limit(6);
  ...
  titulo.textContent = `Mais em ${p.subcategorias.nome}`;
  ...
  grid.innerHTML = relacionados.map(r => {
    ...
    const precoFmt = 'R$ ' + final.toFixed(2).replace('.', ',');
    return `
      <div class="rel-card" onclick="abrirModal(${r.id})">
        ...
        <div class="rel-card-info">
          ...
          <div class="rel-card-preco">${precoFmt}${desc > 0 ? ` <small style="color:#70c880;font-size:10px">-${desc}%</small>` : ''}</div>
...
async function toggleFavModal() {
  ...
  // Reutiliza a função do favoritar.js
  await toggleFav(btn, produtoAtual.id);
  // Sincroniza o coração do card na página também
